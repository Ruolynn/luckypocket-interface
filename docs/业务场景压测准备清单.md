...
- [ ] 在 `.env.loadtest` 中开启 `LOADTEST_MODE=true`，可选配置 `LOADTEST_TOKEN_DECIMALS`、`LOADTEST_AUTO_CREATE_RECIPIENT`
...
| 创建礼物 | `scripts/load-test/k6-create-gift.js` | 需要 `LOADTEST_MODE=true`，并配置 JWT/SENDER 信息 |
| 领取礼物 | `scripts/load-test/k6-claim-gift.js` | 需预置待领取礼物，配置 `CLAIM_USER_ID`/`CLAIM_ADDRESS` |
| Frame 领取 | `scripts/load-test/k6-frame-claim.js` | 配置 `FRAME_TARGETS=packetId:fid;...` |
